...
static unsigned char WritePhyReg(unsigned char,
                                 unsigned int); // write to a phy reg
static unsigned int WriteMacBuffer(unsigned char *,
                                   unsigned int); // write to mac buffer
...
/** MACRO for selecting or deselecting chip select for the ENC28J60. Some HW
 * dependancy.*/
...
void initMAC(const unsigned char *deviceMAC)
{
  ...
  WriteCtrReg(MAIPGL,
              0x12); // non back to back interpacket gap. set as per data sheet
  WriteCtrReg(MAIPGH, 0x0C);
  WriteCtrReg(MABBIPG,
              0x12); // back to back interpacket gap. set as per data sheet
  WriteCtrReg(MAMXFLL,
              (unsigned char)(MAXFRAMELEN & 0x00ff)); // set max frame len
...
unsigned int MACWrite(unsigned char *packet, unsigned int len)
{
  ...
  WriteCtrReg(ERDPTL,
              (unsigned char)(len & 0x00ff)); // Setup the buffer read ptr to
                                              // read status struc
 ...
 *
 * Description: If a valid packet is available in the ENC28J60 this function
 reads the packet into a
 *              buffer. The memory within the ENC28J60 will then be released.
 This version of the driver does not use interrupts so this function needs to be
 polled.\n \n
 *
 ...
 * 4) First two bytes contain the ptr to the start of next packet. Read this
 value in. \n
...
/** \brief ReadETHReg.
 *
 * Description: Reads contents of the addressed ETH reg over SPI bus. Assumes
...
/** \brief ReadMacReg.
 *
 * Description: Read contents of addressed MAC register over SPI bus. Assumes
 * correct bank selected.
 *
 * \author Iain Derrington
 ...
 *
 * Description:  Writing to PHY registers is different to writing the other
 regeisters in that the registers can not be accessed directly. This function
 wraps up the requirements for writing to the PHY reg. \n \n
 ...
 *
 * Description: No direct access allowed to phy registers so the folling process
 * must take place. \n \n 1) Write address of phy reg to read from into
 ...
 *
 * Description: Writes a byte to the address register. Assumes that correct bank
 * has all ready been selected
 *
 * \author Iain Derrington
 ...
 *
 * Description: Reads a number of bytes from the ENC28J60 internal memory.
 * Assumes auto increment is on.
 *
 * \author Iain Derrington
 ...
 *
 * Description: Writes a number of bytes to the ENC28J60 internal memory.
 * Assumes auto increment is on.
 *
 * \author Iain Derrington
 * \param bytBuffer
...
/** \brief Set bit field.
 *
 * Description: Sets the bit/s at the address register. Assumes correct bank has
 * been selected.
 *
 * \author Iain Derrington
 ...
 *
 * Description: Clears the bit/s at the address register. Assumes correct bank
 * has been selected.
 *
 * \author Iain Derrington
 * \param bytAddress Address of registed where bit is to be set
