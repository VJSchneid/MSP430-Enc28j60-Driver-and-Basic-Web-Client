  ...
  *dnsq++ = 1;
  ...
  while (1)
  {
    ...
    if (((UDPhdr *)packet)->sourcePort == HTONS(DNSUDPPORT))
    {
      ...
      {
        // IP address is after original query so we should go to the end
        // plus lenOfQuery There are also 12 bytes of other data we do
        // not need to know Grab IP from message and copy into serverIP
        // The address should be the last 4 bytes of the buffer. This is
        // not fool proof and should be changed in the future
...
int IPstackHTMLPost(const char *url, const char *data, char *reply)
{
  ...
  memset(packet, 0,
         sizeof(TCPhdr)); // zero the memory as we need all flags = 0
